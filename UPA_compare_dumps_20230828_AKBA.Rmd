...
data_cut_2 <- as.Date("20230825",format="%Y%m%d")
...
new_path = "~/GitHub/upa_data_dump/dump/UPA_20230908/"
...
d_new_pre$unique_prescription_id = paste0(d_new_pre[,cols<-grep("nique_patient_id",colnames(d_new_pre))],"/",d_new_pre$Prescription,"_",d_new_pre[,cols<-grep("start",colnames(d_new_pre))])
d_old_pre$unique_prescription_id = paste0(d_old_pre[,cols<-grep("nique_patient_id",colnames(d_old_pre))],"/",d_old_pre[,cols<-grep("name",colnames(d_old_pre))],"_",d_old_pre[,cols<-grep("start",colnames(d_old_pre))])
...
# Filtrering
unique_id_old = intersect(x=unique_id_old ,y=unique(d_old_pre[d_old_pre$drugname %in% drugs 
                                                          ...
                                                          >= as.Date(as.character(20190101),format="%Y%m%d")),]$unique_patient_id))

unique_id_new = intersect(x=unique_id_new ,y=unique(d_new_pre[d_new_pre$drugname %in% drugs 
...
pat_new = pat_new[pat_new$unique_patient_id %in% unique_id_new,]
pat_old = pat_old[pat_old$unique_patient_id %in% unique_id_old,]
  
d_new_pre=d_new_pre[d_new_pre$unique_patient_id %in% unique_id_new,]
d_old_pre=d_old_pre[d_old_pre$unique_patient_id %in% unique_id_old,]

if(include_SAE==TRUE){
  d_new_sae=d_new_sae[d_new_sae$unique_patient_id %in% unique_id_new,]
  d_old_sae=d_old_sae[d_old_sae$unique_patient_id %in% unique_id_old,]
}

d_new_visit=d_new_visit[d_new_visit$unique_patient_id %in% unique_id_new,]
d_old_visit=d_old_visit[d_old_visit$unique_patient_id %in% unique_id_old,]
...
d_new_pre = unique(d_new_pre[,c("unique_patient_id","drugname","drugstartdate","registrationdate","unique_prescription_id")])
d_old_pre = unique(d_old_pre[,c("unique_patient_id","drugname","drugstartdate","registrationdate","unique_prescription_id")])
...
SAE_gained <-setdiff(d_new_sae$sae_id,d_old_sae$sae_id)
SAE_lost   <-setdiff(d_old_sae$sae_id,d_new_sae$sae_id)

SAE_overlap <- intersect(d_old_sae$sae_id,d_new_sae$sae_id)

df_lost_sae<-d_old_sae[d_old_sae$sae_id %in% SAE_lost,]
df_gain_sae<-d_new_sae[d_new_sae$sae_id %in% SAE_gained,]

df_same_sae<-d_new_sae[d_new_sae$sae_id %in% SAE_overlap,]
...
df_gain_sae <- df_gain_sae[,colnames(df_gain_sae) %in% c("unique_sae_id","saestartdate","saestopdate","saename","sae_id")]
...
for(pid in unique(df_lost_sae$patient_id)){
  #print(pid)
  #print(pat_old$statusdiscontinuereasonuserdecided[pat_old$patient_id == pid])
  df_lost_sae$review_state_patient[df_lost_sae$patient_id == pid] = unique(pat_old$statusdiscontinuereasonuserdecided[pat_old$patient_id == pid])[1]
}
...
df_lost_sae<-df_lost_sae[,colnames(df_lost_sae) %in% c("unique_sae_id","sae_id","saestartdate","saestopdate","saename","diagnosis","review_state_patient","lost_patient")]
...
d_pat_loss=pat_old[pat_old$socialsecuritynumber %in% pat_loss,]
...
pat_date_var=d_pat_loss[colnames(d_pat_loss) %in% c("monthyearofdiagnosis","monthyearstartofsymptoms",
                                                    "statusdiscontinuedatenationalregistrysync","statusdiscontinuedateuserdecided")]
print("###########################################################################################")
print("###################################### PATIENT LEVEL ######################################")
print("###########################################################################################")
print(paste("# patients = ",length(d_pat_loss)))
for(x in colnames(pat_date_var)){
  #values<-d_pat_loss[!is.na(d_pat_loss[,col<-x]),col<-x]
  #print(x)
  #tem<-hist(values,breaks=10,main=x,xlab=x)
  #counts<-append(NA,tem$counts)
  #print(tem$breaks)
  #print(counts)
  values<-na.omit(parse_date(d_pat_loss[,col<-x]))
  hist(values,breaks=10,main=x,xlab=x)
  print("=======================================================================================")
}
...
for(x in colnames(pre_date_var)){
  #values<-d_pre_loss[!is.na(d_pre_loss[,col<-x]),col<-x]
  #print(x)
  #tem<-hist(values,breaks=10,main=x,xlab=x)
  #counts<-append(NA,tem$counts)
  #print(tem$breaks)
...
for(x in colnames(sae_date_var)){
  #values<-d_sae_loss[!is.na(d_sae_loss[,col<-x]),col<-x]
  #print(x)
  #tem<-hist(values,breaks=10,main=x,xlab=x)
  #counts<-append(NA,tem$counts)
  #print(tem$breaks)
...
for(x in colnames(vis_date_var)){
  #values<-d_visit_loss[!is.na(d_visit_loss[,col<-x]),col<-x]
  #print(x)
  #tem<-hist(values,breaks=10,main=x,xlab=x)
  #counts<-append(NA,tem$counts)
  #print(tem$breaks)
...
for(x in colnames(year_date_var)){
  #values<-d_yearly_loss[!is.na(d_yearly_loss[,col<-x]),col<-x]
  #print(x)
  #tem<-hist(values,breaks=10,main=x,xlab=x)
  #counts<-append(NA,tem$counts)
  #print(tem$breaks)
...
## 11. Content overview of categorical variabels for lost patients
```{r,eval=TRUE, echo=F}
...
print("################################# PATIENT TABEL #################################")
print(paste("# lost patients:",length(unique(pat_cat_loss$socialsecuritynumber))))
