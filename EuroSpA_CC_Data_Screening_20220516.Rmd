...
#filename <- "../datasets/SI/data_si_20220217_v10.txt"
...
#filename = paste("../datasets/","ES/biobad_eurospamay22v5_SHR.txt",sep="")
#metadata <- paste("../datasets/","metadata_ES_study_A2_A3_v1_20220419.xlsx",sep="")

#filename = paste("../datasets/","ET/Estonia 11.08.2022v4.txt",sep="")
...
filename <- "../datasets/IS/bdmards_dt_merge_washed_IS_20220621.csv"
metadata = "../datasets/IS/metadata_master_is_raw_study_A2_A3_v1_0.xlsx"

#filename <- "../datasets/DK/bdmards_dt_merge_washed_DK_20220525.csv"
#metadata = "../datasets/DK/metadata_master_dk_raw_study_A2_A3_v1_0.xlsx"

#filename <- "../datasets/IT/data_italy_20220723_8.txt"
#metadata = "../datasets/IT/metadata_master_raw_study_A2_A3 version_1.0_Italy_20220608_v2.0.xlsx"
...
for(i in 1:3){
  ...
  for(pid in unique_ids){
    if(max(d$bio_drug_series_number[d$patient_id==pid])==i){
...
dosematchpercent=c()
intervalmatchpercent=c()

dosevariansmatch=c()
intervalvariansmatch=c()

dosevarianspercent=c()
intervalvarianspercent=c()
...
for(name in drug_names){
  ...
  for(i in 1:nrow(sub)){
    ...
    if(name=="abatacept"){
      if(diagnosis=="psa"){
        if(!(is.na(sub[i,"weight"])) && length(sub[i,"weight"])!=0){
          if(as.integer(sub[i,"weight"])<60){
            dose=c(125,500)
          }else if(as.integer(sub[i,"weight"])>=60 | as.integer(sub[i,"weight"])<=100){
            dose=c(125,750)
          }else if( as.integer(sub[i,"weight"])>100){
            dose=c(125,1000)
          }
        }else{
     ...
     interval=14 
      ...
      if(diagnosis=="psa"){
        if(!(is.na(sub[i,"weeks_delta_to_baseline"])) && sub[i,"weeks_delta_to_baseline"]==0){
      ...
      if(diagnosis=="spa"){
        if(!(is.na(sub[i,"weeks_delta_to_baseline"])) && sub[i,"weeks_delta_to_baseline"]==0){
          dose=400
          interval=c(1,14,28)
        }else if(!(is.na(sub[i,"weeks_delta_to_baseline"])) && sub[i,"weeks_delta_to_baseline"]>=2){
          dose=200
          interval=14
        }else if(!(is.na(sub[i,"weeks_delta_to_baseline"])) && sub[i,"weeks_delta_to_baseline"]>=4){
          dose=400
          interval=28
        }
    ...
    if(length(data_interval)!=0 && !(is.na(data_interval))){
  ...
  if(name %in% unique(d$bio_drug_name)){
    ...
    # calculate percentage match (and save percentage match for color calculation)
    dose_match_percent=dose_match/length(sub$patient_id)*100
    interval_match_percent=interval_match/length(sub$patient_id)*100
    
    dosematchpercent<-append(dosematchpercent,dose_match_percent)
    intervalmatchpercent<-append(intervalmatchpercent,interval_match_percent)
    
    # calculate amount of matches within the variance
    ...
    # calculate percentage match within the variance (and save percentage match for color calculation)
    ...
    dosevarianspercent<-append(dosevarianspercent,dose_varians_percent)
    intervalvarianspercent<-append(intervalvarianspercent,interval_varians_percent)
    
    # write entry as <n(%)> format for the table
    ...
    # append the formated number and percentage matches to the correct lists for the dataframe
    ...
    # saves the names of the drugs used in the data set
...
if(length(final_names)!=0){
  ...
  for(i in 1:length(colnames(df))){
    if(i==2){
      colorlist=dosematchpercent
    }else if(i==3){
      colorlist=dosevarianspercent
    }else if(i==4){
      colorlist=intervalmatchpercent
    }else if(i==5){
      colorlist=intervalvarianspercent
    }
    if(i!=1){
      for(j in 1:length(rownames(df))){
        if(colorlist[j]>=87.5){
          suppressWarnings(df[j,i]<-color_tile("palegreen3","palegreen3")(df[j,i]))
        }else if(colorlist[j]<87.5 && colorlist[j]>=75){
          suppressWarnings(df[j,i]<-color_tile("palegreen2","palegreen2")(df[j,i]))
        }else if(colorlist[j]<75 && colorlist[j]>=62.5){
          suppressWarnings(df[j,i]<-color_tile("palegreen1","palegreen1")(df[j,i]))
        }else if(colorlist[j]<62.5 && colorlist[j]>=50){
          suppressWarnings(df[j,i]<-color_tile("palegreen","palegreen")(df[j,i]))
        }else if(colorlist[j]<50 && colorlist[j]>=37.5){
          suppressWarnings(df[j,i]<-color_tile("peachpuff","peachpuff")(df[j,i]))
        }else if(colorlist[j]<37.5 && colorlist[j]>=25){
          suppressWarnings(df[j,i]<-color_tile("peachpuff1","peachpuff1")(df[j,i]))
        }else if(colorlist[j]<25 && colorlist[j]>=12.5){
          suppressWarnings(df[j,i]<-color_tile("peachpuff2","peachpuff2")(df[j,i]))
        }else if(colorlist[j]<12.5 && colorlist[j]>=0){
          suppressWarnings(df[j,i]<-color_tile("peachpuff3","peachpuff3")(df[j,i]))
        }
      }
    }
  }
  ...
  print("insufficient data included in the dataset. No standard bDMARDs were used")
}

```

## 11 patients with date_of_last_visits not mapped on a visit
```{r, eval=TRUE, echo=FALSE, fig.height = 15, fig.width = 15}
d$approx_visit_date = as.Date(d$bio_drug_start_date,format="%Y-%m-%d") + d$weeks_delta_to_baseline * 7

num = 0
lll = data.frame()
dup_rows = data.frame()
for(p in unique(d$patient_id)){
  all_vis_for_p = d[d$patient_id == p,]
  max_bdsn = max(all_vis_for_p$bio_drug_series_number, na.rm = T)
  max_weeks = max(all_vis_for_p[all_vis_for_p$bio_drug_series_number == max_bdsn,]$weeks_delta_to_baseline, na.rm = T)
 
  last_visit = all_vis_for_p[all_vis_for_p$weeks_delta_to_baseline == max_weeks & all_vis_for_p$bio_drug_series_number == max_bdsn,]

  if(!is.na(all_vis_for_p$date_of_last_visit[1]) && ((last_visit$approx_visit_date[1])  > (as.Date((all_vis_for_p$date_of_last_visit[1]),format="%Y-%m-%d") + 8))){
    print(last_visit$approx_visit_date[1] + 14)
    print(all_vis_for_p$date_of_last_visit[1])
    print(" ")
    num = num + 1
    lll = rbind(lll,head(all_vis_for_p[,c("patient_id","approx_visit_date","date_of_last_visit")],1))
  }
}
print(num)
```

## 12. Patients with more than one date_of_last_visit
```{r, eval=TRUE, echo=FALSE, fig.height = 15, fig.width = 15}
for(p in unique(d$patient_id)){
  all_vis_for_p = d[d$patient_id == p,]
  if(sum(!is.na(unique(all_vis_for_p$date_of_last_visit))) > 1){
  print("d")
  }
}
```
...
## 13. Patients that have not been followed from the beginning
```{r, eval=TRUE, echo=FALSE, fig.height = 15, fig.width = 15}
if(unique(d$country)=="ET"){
  date_of_registry_start = 20131001
  
  unique(d[d$date_of_first_visit > (d$bio_drug_start_date + 14) & d$date_of_first_visit <date_of_registry_start,]$bio_drug_start_date)
}
```



## 14. Further ideas for screening that may be considered at a later stage
