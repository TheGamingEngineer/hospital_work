...
#get images and metadata from Slovenia
    ...
    # set and adjust metadata for reading
    metadata_path="/home/alexanderandersen/2_wave/metadata/SL/SL_image_metadata_eurospa_imaging_second_wave.xlsx"
...
# make lists of patient ID's for each diagnosis group
patient_archieve_axspa=list(np.unique(excel_metadata.patient_id[excel_metadata.diagnosis_group=="axspa"]))
patient_archieve_psa=list(np.unique(excel_metadata.patient_id[excel_metadata.diagnosis_group=="psa"]))
patient_archieve_other=list(np.unique(excel_metadata.patient_id[(excel_metadata.diagnosis_group!="axspa")&(excel_metadata.diagnosis_group!="psa")]))

# make lists of examination ID's for each diagnosis group
ex_archieve_axspa=list(excel_metadata.patient_id_abcde[excel_metadata.diagnosis_group=="axspa"])
ex_archieve_psa=list(excel_metadata.patient_id_abcde[excel_metadata.diagnosis_group=="psa"])
ex_archieve_other=list(excel_metadata.patient_id_abcde[(excel_metadata.diagnosis_group!="axspa")&(excel_metadata.diagnosis_group!="psa")])
...
patient_diagnosis={}
for x in list(np.unique(excel_metadata.patient_id)):
    if x not in patient_diagnosis and "{" not in x:
        patient_diagnosis[x]=list(excel_metadata.diagnosis_group[excel_metadata.patient_id==x])[0]

# use the patient IDs as a bridge to count the number of images per diagnosis group
for key in image_counting:
    if key in patient_diagnosis.keys():
        if patient_diagnosis[key]=="axspa":
            image_axspa_count+=image_counting[key]
        elif patient_diagnosis[key]=="psa":
...
axspa_metadata=excel_metadata[excel_metadata.diagnosis_group=="axspa"]
psa_metadata=excel_metadata[excel_metadata.diagnosis_group=="psa"]
other_metadata=excel_metadata[(excel_metadata.diagnosis_group!="axspa") & (excel_metadata.diagnosis_group!="psa")]

axspa_pat_dict["SIJ"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.xray_sij==1)|(axspa_metadata.mri_sij==1)]))
psa_pat_dict["SIJ"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.xray_sij==1)|(psa_metadata.mri_sij==1)]))
other_pat_dict["SIJ"]=len(np.unique(other_metadata.patient_id[(other_metadata.xray_sij==1)|(other_metadata.mri_sij==1)]))
total_pat_dict["SIJ"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.xray_sij==1)|(excel_metadata.mri_sij==1)]))

axspa_pat_dict["Cervical Spine"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.xray_cervical_spine==1)|(axspa_metadata.mri_cervical_spine==1)]))
psa_pat_dict["Cervical Spine"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.xray_cervical_spine==1)|(psa_metadata.mri_cervical_spine==1)]))
other_pat_dict["Cervical Spine"]=len(np.unique(other_metadata.patient_id[(other_metadata.xray_cervical_spine==1)|(other_metadata.mri_cervical_spine==1)]))
total_pat_dict["Cervical Spine"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.xray_cervical_spine==1)|(excel_metadata.mri_cervical_spine==1)]))

axspa_pat_dict["Lumbar Spine"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.xray_lumbar_spine==1)|(axspa_metadata.mri_lumbar_spine==1)]))
psa_pat_dict["Lumbar Spine"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.xray_lumbar_spine==1)|(psa_metadata.mri_lumbar_spine==1)]))
other_pat_dict["Lumbar Spine"]=len(np.unique(other_metadata.patient_id[(other_metadata.xray_lumbar_spine==1)|(other_metadata.mri_lumbar_spine==1)]))
total_pat_dict["Lumbar Spine"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.xray_lumbar_spine==1)|(excel_metadata.mri_lumbar_spine==1)]))

axspa_pat_dict["Thoracic Spine"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.mri_thoracic_spine==1)]))
psa_pat_dict["Thoracic Spine"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.mri_thoracic_spine==1)]))
other_pat_dict["Thoracic Spine"]=len(np.unique(other_metadata.patient_id[(other_metadata.mri_thoracic_spine==1)]))
total_pat_dict["Thoracic Spine"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.mri_thoracic_spine==1)]))

if "mri_sij_STIR_semi_coronal" in excel_metadata:
    axspa_pat_dict["SIJ STIR semi-coronal"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.mri_sij_STIR_semi_coronal==1)]))
    psa_pat_dict["SIJ STIR semi-coronal"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.mri_sij_STIR_semi_coronal==1)]))
    other_pat_dict["SIJ STIR semi-coronal"]=len(np.unique(other_metadata.patient_id[(other_metadata.mri_sij_STIR_semi_coronal==1)]))
    total_pat_dict["SIJ STIR semi-coronal"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.mri_sij_STIR_semi_coronal==1)]))

if "mri_sij_T1_semi_coronal" in excel_metadata:
    axspa_pat_dict["SIJ T1 semi-coronal"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.mri_sij_T1_semi_coronal==1)]))
    psa_pat_dict["SIJ T1 semi-coronal"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.mri_sij_T1_semi_coronal==1)]))
    other_pat_dict["SIJ T1 semi-coronal"]=len(np.unique(other_metadata.patient_id[(other_metadata.mri_sij_T1_semi_coronal==1)]))
    total_pat_dict["SIJ T1 semi-coronal"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.mri_sij_T1_semi_coronal==1)]))

axspa_pat_dict["Spine"]=len(np.unique(axspa_metadata.patient_id[(axspa_metadata.mri_cervical_spine==1)&(axspa_metadata.mri_thoracic_spine==1)&(axspa_metadata.mri_lumbar_spine==1)]))
psa_pat_dict["Spine"]=len(np.unique(psa_metadata.patient_id[(psa_metadata.mri_cervical_spine==1)&(psa_metadata.mri_thoracic_spine==1)&(psa_metadata.mri_lumbar_spine==1)]))
other_pat_dict["Spine"]=len(np.unique(other_metadata.patient_id[(other_metadata.mri_cervical_spine==1)&(other_metadata.mri_thoracic_spine==1)&(other_metadata.mri_lumbar_spine==1)]))
total_pat_dict["Spine"]=len(np.unique(excel_metadata.patient_id[(excel_metadata.mri_cervical_spine==1)&(excel_metadata.mri_thoracic_spine==1)&(excel_metadata.mri_lumbar_spine==1)]))
...
diagnosis_data_rows=["SIJ",
                     ...
                     "SIJ T1 semi-coronal",
                     "Spine"]                  
diagnosis_data=[[axspa_pat_dict["SIJ"],psa_pat_dict["SIJ"],other_pat_dict["SIJ"],total_pat_dict["SIJ"]],
                [axspa_pat_dict["Cervical Spine"],psa_pat_dict["Cervical Spine"],other_pat_dict["Cervical Spine"],total_pat_dict["Cervical Spine"]],
                [axspa_pat_dict["Lumbar Spine"],psa_pat_dict["Lumbar Spine"],other_pat_dict["Lumbar Spine"],total_pat_dict["Lumbar Spine"]],
                [axspa_pat_dict["Thoracic Spine"],psa_pat_dict["Thoracic Spine"],other_pat_dict["Thoracic Spine"],total_pat_dict["Thoracic Spine"]],
                [axspa_pat_dict["SIJ STIR semi-coronal"],psa_pat_dict["SIJ STIR semi-coronal"],other_pat_dict["SIJ STIR semi-coronal"],total_pat_dict["SIJ STIR semi-coronal"]],
                [axspa_pat_dict["SIJ T1 semi-coronal"],psa_pat_dict["SIJ T1 semi-coronal"],other_pat_dict["SIJ T1 semi-coronal"],total_pat_dict["SIJ T1 semi-coronal"]],
                [axspa_pat_dict["Spine"],psa_pat_dict["Spine"],other_pat_dict["Spine"],total_pat_dict["Spine"]]]    
...
if "mri_sij_STIR_semi_coronal" in excel_metadata:
    BI_header["mri_sij_STIR_semi_coronal"]=0
    BI_names.append("mri_sij_STIR_semi_coronal")
    
if "mri_sij_T1_semi_coronal" in excel_metadata:
    BI_header["mri_sij_T1_semi_coronal"]=0
    BI_names.append("mri_sij_T1_semi_coronal")
    
# Gathers the number of examinations of the various types from the metadata file
for i in BI_names:
    BI_header[i]+=sum(list(excel_metadata[i])[1:])

# calculates the whole spine counts
BI_header["xray_spine"]=len(list(excel_metadata.patient_id[excel_metadata.xray_lumbar_spine==1][excel_metadata.xray_cervical_spine==1]))
BI_header["mri_spine"]=len(list(excel_metadata.patient_id[excel_metadata.mri_lumbar_spine==1][excel_metadata.mri_cervical_spine==1][excel_metadata.mri_thoracic_spine==1]))
BI_names.append("xray_spine")
BI_names.append("mri_spine")

if "mri_sij_STIR_semi_coronal" in excel_metadata and "mri_sij_T1_semi_coronal" in excel_metadata:            
