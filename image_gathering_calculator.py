...
meta=["/home/alexanderandersen/2_wave/metadata/SL/metadata_SL_20230606.xlsx",
      "/home/alexanderandersen/2_wave/metadata/IS/metadata_IS_20230606.xlsx"]  
...
for x in meta:
    ...
    output_header=[variables[0],variables[3].replace('_',''),"start_date_of_first_bdmard"]+variables[4:]
    #exit()
    ...
    for y in patient_ID:
        ...
        for line in metadata:
            # we only want to look at each patient individually
            if type(line[0])!=float and line[0]==y:
                if str(line[3])!="nan":
                    diagnosis=line[3]
                if type(line[2])!=int:
                    if date==21991231:
                        date=line[2]
                elif pd.to_datetime(str(line[2]))<pd.to_datetime(str(date)):
                    date=pd.to_datetime(str(line[2]))
                for i in range(len(var_list)):
                    try:
                        int(line[i+4])
                        var_list[i]+=int(line[i+4])
                    except ValueError:
                        var_list[i]+=0
